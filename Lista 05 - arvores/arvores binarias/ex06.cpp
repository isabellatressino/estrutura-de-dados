/*Construir uma função que verifica quantos nós possuem dois descendentes*/
...
int contar(no_arvb *t){
    if(t == NULL) return 0;
    if(t->le != NULL && t->ld != NULL) return 1 + contar(t->le) + contar(t->ld);
    return contar(t->le) + contar(t->ld);
}
...
int main(){
    ...
    cout << "\nNumero de nos com dois descendentes = " << cont;
